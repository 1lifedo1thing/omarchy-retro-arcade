...
with tempfile.TemporaryDirectory(prefix='arcade-mouse-') as tmp:
    ...
    try:
        ...
        top = 144
        def select(index):
            mouse(right-rail_w/2, top+(index+.5)*(bottom-top)/9)
        def play(name):
        ...
        shot('shelf')
        # All nine entries can be selected, launched and left without a key.
        for index,name in enumerate(['Circuit Pinball','Solitaire','Scram','Invaders','Chess','Stack','Snake','Bubble','Blast']):
        ...
        print('PASS: nine mouse launches/returns, Pinball confirm/cancel, Invaders mouse and keyboard handoff')
