...
impl SnakeApp {
    ...
    fn board(&self, ui: &mut egui::Ui) -> Rect {
        ...
        let cell = ((available.x - 12.) / WIDTH as f32)
            .min((available.y - 32.) / HEIGHT as f32)
            .max(2.);
        let size = Vec2::new(cell * WIDTH as f32, cell * HEIGHT as f32);
        let origin = ui.cursor().min + Vec2::new((available.x - size.x) * 0.5, 4.);
        ...
        painter.rect_filled(board.expand(5.), 5., edge);
